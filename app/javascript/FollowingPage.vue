        ...
        <router-link :to="{name: 'user-show',params: {id: e.id}}">
          <div class="user-list-image pb-3">
            <div v-if="e.image.url">
              <img :src= e.image.url class="user-list-icon mt-3">
            </div>
            <div v-else>
          ...
          <div class="user-list_body">
            ...
            <div class="user-list_text mt-5">
              <div v-if="e.text && e.text.length<=12">{{e.text}}</div>
              <div v-else-if="e.text && e.text.length>12">{{e.text.slice(0,12) + '...'}}</div>
...
export default {
  data() {
    return {
      keyword: this.$store.state.keyword_follow,
      users: [],
      parPage: 16,
